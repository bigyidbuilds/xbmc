...
class PodcastRss():
	...
	def Channel(self,url):
		...
		except:Log(({'title':feed.title.encode('utf8'),'description':description.encode('utf8'),'image':image,'rss_url':url}))
		...
		if str(amount).isdigit():
			...
			for i in range(amount):
				...
				try:
					...
					description = ''
				self.ITEM.append({'title':entries[i].title.encode('utf8'),'image':image,'description':description.encode('utf8'),'date':date,'playlink':playlink,'podid': ''.join(feed.title.encode('utf8').lower().split())})
		elif amount=='all':
			for entry in entries:
				...
				try:
					...
					description = ''
				self.ITEM.append({'title':entry.title.encode('utf8'),'image':image,'description':description.encode('utf8'),'date':date,'playlink':playlink,'podid': ''.join(feed.title.encode('utf8').lower().split())})
		else:
			pass
