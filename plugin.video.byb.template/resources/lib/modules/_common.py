...
def AddDir(name,url,mode,iconimage,fanart,description,genre,date,credits,isFolder=True,iconbackup="DefaultFolder.png"):
	u=sys.argv[0]+"?url="+urllib.quote_plus(url)+"&mode="+str(mode)+"&name="+urllib.quote_plus(name)+"&fanart="+urllib.quote_plus(fanart)+"&icon="+urllib.quote_plus(iconimage)
	ok=True
	liz=xbmcgui.ListItem(name)
	liz.setArt({'thumb':iconimage,'icon':iconbackup,'fanart':fanart})
